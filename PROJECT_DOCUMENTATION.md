...
- 監控模板提供 A 組 `EMA8/21`、B 組 `EMA8/34 + 跨日暖機`、C 組 `EMA 單因子阻擋 + 突破續強確認`；三組皆關閉內部市場狀態過濾，供同資料 A/B/C 比較。
- C 組已把 EMA-only 多單改為硬阻擋，且預設開啟突破後一根 K 確認；EMA 多頭若沒有 RSI LONG 或「前一根 VolumeBreakout、下一根續收高」一起確認，scanner 會保留阻擋原因供批次回測比較。
...
2. 用 A/B/C 模板比較 EMA8/21、EMA8/34 跨日暖機與 EMA 單因子阻擋後的條件貢獻。
