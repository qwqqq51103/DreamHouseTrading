...
- 監控模板：A 組 `EMA8/21`、B 組 `EMA8/34 + 跨日暖機`、C 組 `EMA 單因子阻擋 + 突破續強確認` 可直接切換做 SQL 回測比較。
...
- SQL 雷達回測若套用 B/C 模板，會從 SQL 載入前期 K 線暖機慢速 EMA，但只在指定交易日產生交易與績效；監控設定也可手動開關跨日暖機並調整暖機 K 線根數。
- C 組用於測試另一種收斂方式：EMA 多頭本身只算結構，若沒有 RSI 轉強或「前一根有效放量突破、下一根續收高」確認，雷達會阻擋自動進場並在回測阻擋原因中記錄；模板預設開啟「突破後一根 K 確認」，監控設定的「阻擋 EMA 單因子進場」會隨模板套用與自訂模板儲存。
